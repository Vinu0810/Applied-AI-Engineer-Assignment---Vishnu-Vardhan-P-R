...
def ai_summary(df, insights, ttests):
    """
    Generate a detailed executive summary from data, insights, and statistical tests.
    """

    # Context from insights.json
    text = "Here are some app market insights:\n"
    for ins in insights["insights"]:
        text += f"- {ins['title']}: {ins['insight']} (confidence {ins['confidence']})\n"

    # Add statistical test results
    text += "\nStatistical validation results:\n"
    for t in ttests:
        text += f"- {t['title']}: {t['insight']} (p-value {t['supporting_stats']['Probability of observing the difference (p-value)']})\n"
    ...
    - Content ratings: {df['content_rating'].nunique()}
    - Platforms: {df['store'].unique().tolist()}
    """

    # Instruction to AI for style
    text += """
    Write a comprehensive executive summary for business leaders.
    - Use 5–7 sections.
    - Cover category dominance, free vs paid, content ratings, platform distribution, update frequency, app size, and business opportunities.
    - Provide both insights and strategic recommendations.
    - Write in clear, professional business language.
    - Also write about the best apps and also on how last update affects the rating etc.
    """
...
def ai_qa(df, insights, ttests, question):
    """
    Answer business questions using dataset stats, insights, and validation results.
    """
    # Summarize dataset-level stats
    dataset_stats = f"""
    Dataset overview:
    - Total apps: {len(df)}
    - Categories: {df['category'].nunique()}
    ...
    for ins in insights["insights"]:
        insight_text += f"- {ins['title']}: {ins['insight']} (confidence {ins['confidence']})\n"

    # Add t-test results
    validation_text = "Statistical validation results:\n"
    for t in ttests:
        validation_text += f"- {t['title']}: {t['insight']} (p-value {t['supporting_stats']['Probability of observing the difference (p-value)']})\n"
    ...
    Answer clearly, concisely, and in business-friendly language.
    """
